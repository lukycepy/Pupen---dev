...
export default function GuidelinesTab({ lang }: { lang: string }) {
  const isEn = lang === 'en';
  const { title, html } = useSitePageContent('pravidla', lang);
  return (
    <div className="bg-white p-10 rounded-[3rem] border border-stone-100 shadow-sm animate-in fade-in duration-500">
      ...
      {html ? (
        <div className="mt-8 bg-stone-50 border border-stone-100 rounded-[2rem] p-8">
          {title ? <div className="text-xl font-black text-stone-900 mb-4">{title}</div> : null}
        ...
        <div className="mt-8 grid lg:grid-cols-12 gap-8">
            ...
            {[
              {
                ...
                body: isEn ? 'No harassment, hate speech or discrimination.' : 'Žádné obtěžování, nenávistné projevy ani diskriminace.',
              },
              {
                title: isEn ? 'No spam' : 'Žádný spam',
                body: isEn ? 'Do not send unsolicited promotions or repetitive messages.' : 'Neposílejte nevyžádané promo ani opakované zprávy.',
              },
              {
                title: isEn ? 'Privacy first' : 'Soukromí na prvním místě',
                body: isEn ? 'Do not share personal data without consent.' : 'Nesdílejte osobní údaje bez souhlasu.',
              },
              {
                title: isEn ? 'Report issues' : 'Nahlašujte problémy',
                body: isEn ? 'Use “Report” if you see problematic behavior.' : 'Použijte „Nahlásit“, pokud uvidíte problémové chování.',
              },
            ].map((s) => (
          ...
          <div className="lg:col-span-5">
            <div className="bg-green-50 border border-green-100 rounded-[2rem] p-6">
              <div className="text-[10px] font-black uppercase tracking-widest text-green-700 mb-2">{isEn ? 'Moderation' : 'Moderace'}</div>
