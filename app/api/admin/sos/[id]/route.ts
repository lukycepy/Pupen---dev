...
export async function PATCH(req: Request, ctx: any) {
  try {
    ...
    for (const k of ['title', 'category', 'phone', 'email', 'url', 'note', 'sort_order']) {
      if (patch[k] !== undefined) payload[k] = patch[k];
    }
    if (patch.is_public !== undefined) payload.is_public = !!patch.is_public;

    const supabase = getServerSupabase();
    const up = await supabase.from('sos_contacts').update(payload).eq('id', id).select('*').single();
...
export async function DELETE(req: Request, ctx: any) {
  try {
    ...
    const del = await supabase.from('sos_contacts').delete().eq('id', id);
