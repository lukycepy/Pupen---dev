...
interface SmokeOtazkyResponse {
  ok: boolean;
  checks: Record<string, SmokeCheckResult>;
}
...
export async function GET(req: Request) {
  ...
  const out: SmokeOtazkyResponse = { ok: true, checks: {} };
  ...
  try {
    ...
    out.checks.waitlistConfig = { ok: false, error: getErrorMessage(error) };
    out.ok = false;
  }
  ...
  try {
    ...
    out.checks.newsletterDoi = { ok: false, error: getErrorMessage(error) };
    out.ok = false;
  }
  ...
  try {
    ...
    out.checks.rsvpTable = { ok: false, error: getErrorMessage(error) };
    out.ok = false;
  }
  ...
  try {
    ...
    out.checks.newsletterSubscriptionsTable = { ok: false, error: getErrorMessage(error) };
    out.ok = false;
  }
