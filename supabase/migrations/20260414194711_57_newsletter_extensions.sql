...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public' AND tablename = 'newsletter_templates' AND policyname = 'Admin all templates'
    ) THEN
        EXECUTE '
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public' AND tablename = 'newsletter_drafts' AND policyname = 'Admin all drafts'
    ) THEN
        EXECUTE '
